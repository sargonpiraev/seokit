...
export async function getPokemon(slug: string, locale: string): Promise<LocalizedPokemon> {
  ...
  const species = await pokeFetch<PokemonSpecies>(`/pokemon-species/${pokemon.species.name}`)
...
export async function getGeneration(slug: string, locale: string): Promise<LocalizedGeneration> {
  ...
  return {
    ...
    pokemonSlugs: generation.pokemon_species
      ...
      .filter((name) => demoSet.has(name)),
    typeSlugs: generation.types.map((entry) => entry.name).filter((name) => typeSet.has(name)),
  }
}

export async function listDemoGenerations(locale: string): Promise<LocalizedGeneration[]> {
