...
import { breadcrumbJsonLd, itemListJsonLd, thingJsonLd } from '@/lib/json-ld'
import { absolutePageUrl, buildPageMetadata } from '@/lib/metadata'
import { DEMO_TYPES, getPokemon, getType, isDemoType, listDemoTypes } from '@/lib/pokeapi'
...
export function generateStaticParams() {
  return routing.locales.flatMap((locale) => DEMO_TYPES.map((slug) => ({ locale, slug })))
}
...
export default async function TypeDetailPage({ params }: TypeDetailPageProps) {
  ...
  const [type, catalogTypes] = await Promise.all([getType(slug, locale), listDemoTypes(locale)])
  const pokemon = await Promise.all(type.pokemonSlugs.map((name) => getPokemon(name, locale)))
  ...
  return (
    <main className="space-y-8">
      ...
      <section className="space-y-3">
        ...
        <div className="flex flex-wrap gap-1.5">{relationBadges(type.doubleDamageTo)}</div>
      </section>

      <section className="space-y-3">
        <h2 className="text-xl font-semibold tracking-tight">{t('weakAgainst')}</h2>
        <div className="flex flex-wrap gap-1.5">{relationBadges(type.doubleDamageFrom)}</div>
      </section>
