...
export function generateStaticParams() {
  return routing.locales.flatMap((locale) => DEMO_GENERATIONS.map((slug) => ({ locale, slug })))
}
...
export default async function GenerationDetailPage({ params }: GenerationDetailPageProps) {
  ...
  const generation = await getGeneration(slug, locale)
  const pokemon = await Promise.all(generation.pokemonSlugs.map((name) => getPokemon(name, locale)))
  const typeSlugs = [...new Set(pokemon.flatMap((entry) => entry.types))].filter((name) =>
    isDemoType(name)
  )
