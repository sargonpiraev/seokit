...
export function PokemonCatalog({ locale, pokemon }: PokemonCatalogProps) {
  ...
  const filtered = useMemo(() => {
    ...
    return pokemon.filter((entry) => {
      ...
      return entry.name.toLowerCase().includes(needle) || entry.slug.includes(needle)
    })
  ...
  return (
    <div className="space-y-6">
      <div className="grid gap-3 sm:grid-cols-3">
        ...
        <Select value={generation} onValueChange={(value) => setGeneration(value ?? 'all')}>
      ...
      {filtered.length === 0 ? (
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {filtered.map((entry) => (
            <Card key={entry.slug} className="h-full transition-colors hover:border-primary/40">
              <Link href={`/${locale}/pokemon/${entry.slug}`} className="group block">
                <CardHeader className="pb-2">
                  <CardDescription>#{entry.id}</CardDescription>
                  <CardTitle className="text-xl group-hover:underline">{entry.name}</CardTitle>
                </CardHeader>
