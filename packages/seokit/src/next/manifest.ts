...
/** Colocated Playwright specs next to `page.tsx` (SEO lives in `page.spec.ts` under `@seo`). */
const PAGE_SEO_SPEC_SUFFIXES = [
  '/page.seo.spec.ts',
  '/page.seokit.spec.ts',
  '/page.spec.ts',
] as const
const PAGE_SEO_SPEC_FILENAMES = new Set(PAGE_SEO_SPEC_SUFFIXES.map((suffix) => suffix.slice(1)))
...
export function specFileToManifestKey(
  specFilePath: string,
  appDir = 'src/app'
): string | undefined {
...
export function inferRouteFromSpec(
  ...
  if (!manifestKey) {
    throw new Error(`Could not map colocated seokit spec to a Next page route: ${specFilePath}`)
  }
...
export function discoverSeokitSpecRoutes(projectRoot: string, appDir = 'src/app'): string[] {
