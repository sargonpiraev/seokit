...
export function checkSeokitSpecCoverage(
  ...
  for (const route of missingSpecs) {
    warnings.push(`Missing colocated page.spec.ts for Next page route ${route}`)
  }

  for (const route of orphanSpecs) {
    warnings.push(
      `Colocated seokit spec exists for route ${route}, but it is not present in Next build manifest`
    )
  }
